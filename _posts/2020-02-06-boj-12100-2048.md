---
title: "[자바]백준 12100 2048(Easy)"
date: 2020-02-06 08:26:28 -0400
categories: twozerofoureight
---
...
public class Main {
	...
	static void move (int [][] priorNums, int dir, int count) {
		if(count == 5) {
			...
			if(curMaxVal > answer) answer = curMaxVal;
			return;
		}
			...
			for(int i = 0; i < curNums.length; i++) {
				for(int j = 0; j < curNums.length; j++) {
					if(curNums[j][i] != 0) {
						for(int k = 0; k < j; k++) {
							if(curNums[k][i] != 0) {
								if(!isCombined[k][i] && curNums[k][i] == curNums[j][i] && (curNums[k+1][i] == 0 || k + 1 == j)) {
									curNums[k][i] *= 2;
									isCombined[k][i] = true;
									curNums[j][i] = 0;
									break;
			...
			for(int i = 0; i < curNums.length; i++) {
				for(int j = 0; j < curNums.length; j++) {
					if(curNums[i][j] != 0) {
						for(int k = 0; k < j; k++) {
							if(curNums[i][k] != 0) {
								if(!isCombined[i][k] && curNums[i][k] == curNums[i][j] && (curNums[i][k+1] == 0 || k + 1 == j)) {
									curNums[i][k] *= 2;
									isCombined[i][k] = true;
									curNums[i][j] = 0;
									break;
